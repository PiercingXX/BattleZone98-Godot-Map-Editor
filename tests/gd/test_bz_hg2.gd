...
func _test_parse_one_zone(t, tmp: String) -> void:
	...
	# Plain zone interleave: disk sample (x,z) is world (x,z), no mirror.
	t.eq(hm.data[0], 100, "disk (0,0) is world (0,0)")
	t.eq(hm.data[1], 57394, "disk (1,0) is world (1,0), flag bits kept")
	t.eq(hm.data[20 * 256 + 10], 48590, "disk (10,20) is world (10,20)")
	t.eq(hm.data[20 * 256 + 10] & 0x1FFF, 7630, "13-bit height above 4095")
	t.eq(hm.data[20 * 256 + 10] >> 13, 5, "flag bits 5")
	t.eq(hm.data[255 * 256 + 255], 2000, "disk (255,255) is world (255,255)")
	t.eq(hm.data[255], 1000, "world (255,0) is untouched plateau")
...
func _test_parse_two_zone(t, tmp: String) -> void:
	...
	t.eq(hm.grid_x, 512)
	t.eq(hm.grid_z, 512)
	# Zone interleave is the only reshuffle; disk (x,z) is world (x,z).
	t.eq(hm.data[0], 1111, "disk (0,0) is world (0,0)")
	t.eq(hm.data[5 * 512 + 300], 7777, "disk (300,5) is world (300,5)")
	t.eq(hm.data[300 * 512 + 10], 3333, "disk (10,300) is world (10,300)")
	t.eq(hm.data[400 * 512 + 400], 4444, "disk (400,400) is world (400,400)")
	t.eq(hm.data[256 * 512 + 256], 24776, "flagged cell, disk (256,256)")
	t.eq(hm.data[5 * 512 + 301], 1000, "neighbour untouched")
...
func _test_edit_isolation(t, tmp: String) -> void:
	...
	var result: Dictionary = BzHg2.read_hg2(path)
	var hm: BzHg2.HeightMap = result.get("heightmap") as BzHg2.HeightMap
	var flagged: int = hm.data[256 * 512 + 256]
	...
	t.eq(hm2.data[256 * 512 + 256], flagged, "flag bits on a different cell survive")
	t.eq(hm2.data[0], 1111)
	t.eq(hm2.data[300 * 512 + 10], 3333)
...
func _test_sample_and_slope(t, tmp: String) -> void:
	...
	# The marked samples sit where the file put them: world (0,0) onward.
	t.near(BzHg2.sample_m(hm, 0.0, 0.0), 10.0, 0.0001, "world (0,0) is the marked cell")
	# Uses the raw word including flags — matches Python, not F1 masked r16.
	t.near(BzHg2.sample_m(hm, 2.5, 0.0), 2874.7, 0.0001, "midway into the flagged cell")
	t.near(BzHg2.sample_m(hm, 5.0, 0.0), 5739.4, 0.0001)
	t.near(BzHg2.sample_m(hm, 640.0, 640.0), 100.0, 0.0001)
	var s: PackedFloat64Array = BzHg2.slope(hm)
	t.eq(s.size(), 256 * 256)
	t.near(s[20 * 256 + 245], 0.0, 0.0001, "flat interior")
	t.near(s[128 * 256 + 128], 0.0, 0.0001)
	t.near(s[0], 1146.0213673400685, 0.0001, "numpy.gradient at world (0,0)")
	var mask: PackedByteArray = BzHg2.buildable_mask(hm)
	t.eq(mask[0], 0, "world (0,0) not buildable")
