...
func _test_parse_one_zone(t, tmp: String) -> void:
	...
	t.eq(hm.data[0], 100, "cell (0,0)")
	t.eq(hm.data[1], 57394, "cell (1,0) keeps flag bits")
	t.eq(hm.data[20 * 256 + 10], 48590, "cell (10,20) raw 7630 | 5<<13")
	t.eq(hm.data[20 * 256 + 10] & 0x1FFF, 7630, "13-bit height above 4095")
	t.eq(hm.data[20 * 256 + 10] >> 13, 5, "flag bits 5")
	t.eq(hm.data[255 * 256 + 255], 2000)
...
func _test_parse_two_zone(t, tmp: String) -> void:
	...
	t.eq(hm.grid_x, 512)
	t.eq(hm.grid_z, 512)
	t.eq(hm.data[0], 1111, "(0,0) zone 0")
	t.eq(hm.data[5 * 512 + 300], 7777, "global (300,5) in zone (1,0)")
	t.eq(hm.data[300 * 512 + 10], 3333, "global (10,300) in zone (0,1)")
	t.eq(hm.data[400 * 512 + 400], 4444, "global (400,400) in zone (1,1)")
	t.eq(hm.data[256 * 512 + 256], 24776, "flagged cell at zone origin (1,1)")
	t.eq(hm.data[5 * 512 + 299], 1000, "neighbour of (300,5) untouched")
...
func _test_edit_isolation(t, tmp: String) -> void:
	...
	t.eq(hm2.data[5 * 512 + 300], 8001, "edited cell")
...
func _test_sample_and_slope(t, tmp: String) -> void:
	...
	t.near(BzHg2.sample_m(hm, 0.0, 0.0), 10.0, 0.0001, "sample (0,0)")
	# Uses raw word at (1,0) including flags — matches Python, not F1 masked r16.
	t.near(BzHg2.sample_m(hm, 2.5, 0.0), 2874.7, 0.0001, "sample midway into flagged cell")
	...
	t.near(s[0], 1146.0213673400685, 0.0001, "numpy.gradient at (0,0)")
	var mask: PackedByteArray = BzHg2.buildable_mask(hm)
	t.eq(mask[0], 0, "(0,0) not buildable")
