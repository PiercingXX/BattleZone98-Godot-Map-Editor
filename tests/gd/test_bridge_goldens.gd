...
func _group_terrain_dirty_reencodes() -> void:
	...
	if not FileAccess.file_exists(hg2):
		return
	# 1 zone: zone-major == row-major and there is no mirror, so world cell 100
	# is disk slot 100.
...
func _write_hg2(path: String, zx: int, zz: int) -> void:
	...
	for zzi in zz:
		for zxi in zx:
			for z in _ZONE:
				for x in _ZONE:
					# Plain zone interleave, no mirror — disk x is world x.
...
func _write_mat(path: String, zx: int, zz: int) -> void:
	...
	for zzi in zz:
		for zxi in zx:
			for z in _MAT_ZONE:
				for x in _MAT_ZONE:
					# Same plain zone interleave as the .hg2.
