...
func run(t) -> void:
	...
	t.ok(walk.visible, "Walk stays on the rail")
	t.ok(not grid.visible, "Grid lives on the View tab")
	t.ok(not slope.visible, "Slope lives on the View tab")
	...
	var bar: Node = scene.find_child("ObjectToolsBar", true, false)
	t.ok(bar != null, "viewport hosts ObjectToolsBar")
	t.ok(bar.find_child("Place", true, false) is Button)
	t.ok(bar.find_child("Select", true, false) is Button)
	t.ok(bar.find_child("AddPath", true, false) is Button)
	t.ok(not bar.visible, "ObjectToolsBar hidden with no map")
	var hint: Label = scene.find_child("CloneHint", true, false)
	t.ok(hint != null, "clone hint label exists")
	t.ok(not hint.visible, "clone hint hidden off the clone tool")
	var center: Node = scene.find_child("Center", true, false)
	var minimap: Node = scene.find_child("MinimapPanel", true, false)
	t.ok(minimap != null and minimap.get_parent() == center, "minimap overlays the 3D viewport")
	t.eq(minimap.mouse_filter, Control.MOUSE_FILTER_STOP)
	t.near(minimap.custom_minimum_size.x, 200.0, 0.1)
	t.near(minimap.custom_minimum_size.y, 200.0, 0.1)
	...
	t.ok(bar.visible, "ObjectToolsBar shows with a session")
	var saved_tool := ToolState.tool
	(bar.find_child("Place", true, false) as Button).pressed.emit()
	t.eq(ToolState.tool, "place", "ObjectToolsBar Place switches the tool")
	(bar.find_child("Select", true, false) as Button).pressed.emit()
	t.eq(ToolState.tool, "select", "ObjectToolsBar Select switches the tool")
	ToolState.set_tool("clone")
	t.ok(hint.visible, "clone hint shows on the clone tool")
	t.ok("Ctrl+click" in hint.text)
	ToolState.set_tool("paint")
	scene.set("_show_grid", false)
	scene.call("_apply_grid")
	t.ok(not bool(scene.get("_show_grid")), "paint does not force the terrain grid")
